"""
Делаем погоню приведения за Пакмэном.
При этом перед побегом Пакмэн кричит "Бежим!", а приведение кричит "В погоню!"
Пакмэн перед побегом плавно уменьшается, за тем увеличивается. Затем бежит.
...
actions.set_angle(victim, 180, 500)
...
sprite.remove(text)
actions.set_size_percent(victim, 30, 30, 500)
actions.set_angle(victim, -135, 500)
actions.move_at_angle_dir(victim, 100, 600)
actions.set_size_percent(victim, 100, 100, 500)
