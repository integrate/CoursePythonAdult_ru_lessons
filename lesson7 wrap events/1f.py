"""
При попытке закрытия окна, пакмэн говорит "О, нееееееееет!"
"""
import time
...
@wrap.always(10)
def eat_enemy():
    if sprite.is_visible(enemy):
        sprite.set_angle_to_point(pacman, sprite.get_x(enemy), sprite.get_y(enemy))
        sprite.move_at_angle_dir(pacman, 2)

@wrap.on_close
def on_close():
    sprite.add_text("О, нееееетт!!!!", sprite.get_x(pacman), sprite.get_y(pacman)-25)